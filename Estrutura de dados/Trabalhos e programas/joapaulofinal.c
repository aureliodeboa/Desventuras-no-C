...
INF  recup(LCOVID l ,int k)
{
	...
	if(k<1||k>l->inf.tam)
	{
		printf("POSICAO INVALIDA a");exit(3);
	}
	else
	{
		for(aux=l->prox;i!=k;aux=aux->prox,i++);
		return aux->inf;
	}
}
...
void imprimir(LCOVID l)
{
	...
	LCOVID aux=l;
 	...
 	printf("\n-------------INFORMACOES GERAIS-------------------\n");
    	...
    	printf("DIGITE (1) SE DESEJAR VER TODAS AS NOTIFICACOES\n E (0) PARA VOLTAR AO MENU PRINCIPAL ");
    	scanf("%d",&in);
...
void ordenartudo(LCOVID pl)
{
	LCOVID l;
	criar(&l);
	l->inf.casos=pl->inf.casos;
	l->inf.mortes=pl->inf.mortes;
	ord_por_mes(pl);
	int c1,c2;
	FILA_ENC vf[b];
	for(c2=0;c2<b;c2++)
	...
	for(c1=0;c1<d;c1++)
	{
		...
		for(c2=0;c2<b;c2++)
		{
			...
			{
				ins(&pl,cons_ret(vf[c2]),pl->inf.tam+1);
			}
		}
	}
	pl->inf.casos=l->inf.casos;
	pl->inf.mortes=l->inf.mortes;
	destruir(l);
}
...
int main()
{
	...
	do
	{
		...
		switch (op)
		{
			...
			case 3://MEDIA MOVEL DE UM DETERMINADO PAIS (3) JOAO PAULO\n" 
			...
			case 4: //TAXA DE MORTALIDADE DE 10 PAISES (4) JOAO PAULO\n
     			system("cls");
     			printf("QUAL PAIS : \n");
     			setbuf(stdin,NULL);
     			gets(pais1);
