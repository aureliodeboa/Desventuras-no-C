	   ...
	   if(!novo)
		...
		else
		{
			...
			if(k==1)
			{
				...
				strcpy((*pl)->inf.pais,inf.pais);
				strcpy((*pl)->inf.data,inf.data);
				//pl->inf.pais=inf.pais;
				//pl->inf.data=inf.data;
...
int main()
{
   ...
   printf("QUAL PAIS : \n");
    	...
    	printf("TOTAL DED MORTES %d\n ",l3->inf.mortes);
    	printf("TOTAL DE  MORTESFUNCAO %d",(totmortes(l3)));
    	//imprimir(l3);
